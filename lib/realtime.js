...
// Reject patches that could reach Object.prototype (prototype pollution).
// json-patch-extended walks paths with a bare obj[key] and assigns values with
// a bare obj[key] = value, so neither the traversal nor the write blocks
// dangerous keys. We guard here before validate/apply, checking both the
// pointer (path/from) and the value: __proto__/constructor/prototype are the
// classic pollution keys, and every other Object.prototype member
// (toString, valueOf, __defineGetter__, ...) is reachable the same way.
...
function pointerHasUnsafeKey(pointer) {
    if (typeof pointer !== 'string') {
        return false;
    }
    var segments = pointer.split('/');
    for (var i = 0; i < segments.length; i++) {
        // RFC 6901 unescape so an encoded segment can't slip past the check.
...
// JSON.parse preserves an own "__proto__" key on the parsed object, which
// objOps.add would assign straight through, so a clean pointer is not enough:
// walk the value too.
function valueHasUnsafeKey(value) {
    if (!value || typeof value !== 'object') {
        return false;
    }
    if (Array.isArray(value)) {
        for (var i = 0; i < value.length; i++) {
            if (valueHasUnsafeKey(value[i])) {
                return true;
            }
        }
        return false;
    ...
    for (var j = 0; j < keys.length; j++) {
        if (isUnsafeKey(keys[j]) || valueHasUnsafeKey(value[keys[j]])) {
            return true;
        }
    }
    return false;
...
function patchIsUnsafe(patch) {
    ...
    for (var i = 0; i < patch.length; i++) {
        ...
        if (pointerHasUnsafeKey(op.path) || pointerHasUnsafeKey(op.from)) {
            return true;
        }
        if (Object.prototype.hasOwnProperty.call(op, 'value') && valueHasUnsafeKey(op.value)) {
            return true;
        }
    }
    return false;
...
module.exports = function initRealtime(io, opts) {
    ...
    // Broadcast a change made outside the socket channel (e.g. the HTTP
    // Save/Publish route) to everyone editing the same document, so their
    // shadow doesn't silently fall behind the persisted __v. `clientId` is the
    // saver's realtime id when known, so their own browser ignores the echo.
    function notifyDocPatched(collection, docId, patch, newVersion, clientId) {
        if (!io || !collection || !docId) {
            return;
        }
        if (!isAllowedCollection(collection)) {
            return;
        }
        if (!Array.isArray(patch) || !patch.length || typeof newVersion !== 'number') {
            return;
        }
        if (patchIsUnsafe(patch)) {
            // The stored document already carries a prototype-polluting key
            // (e.g. an HTTP save persisted a body with an own "__proto__").
            // Don't relay it to live editors; they resync from a clean join.
            return;
        }
        io.to(buildRoom(collection, docId)).emit('doc:patched', {
            patch: patch,
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('doc:patch', async function (payload, ack) {
            ...
            try {
                ...
                if (patchIsUnsafe(payload.patch)) {
                ...
                // The realtime channel cannot rename a document: the room, the
                // query, and every client shadow are keyed by docId, so a patch
                // that changes or clears the value at idpath would persist a
                // document no route can address again (idpattern never matches
                // an empty ID). Renames must go through the HTTP save route,
                // which validates the new ID and redirects clients.
                var idpathInBody = idpath.indexOf('body.') === 0 ? idpath.slice(5) : idpath;
                if (_.get(nextBody, idpathInBody) !== docId) {
                    safeAck(ack, { ok: false, reason: 'PATCH_INVALID' });
                    return;
                }
                ...
                history.addModelHistory(History, doc, updated);
    ...
    return {
        notifyDocPatched: notifyDocPatched
    };
};
